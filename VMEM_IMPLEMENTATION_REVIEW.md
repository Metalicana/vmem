...
Existing JSON records were read with `jq`. The new diagnostic/tests below are
authored but not executed; the Mac remains code-only.
...
The step-8 RNG mismatch above cannot explain an earlier step-0 geometry
difference. Possible causes include numerical nondeterminism, unrecorded RNG
consumption/initialization, or unrecorded dependency differences; none is
established by these traces. They do not tell us whether the first generated
RGB frames already differ or only CUT3R's geometry differs. The downloaded
bundle omits `frame_manifest.json`, so compare those existing CECSL records
before requesting more generation.
...
Added [audit_vmem_pairing.py](scripts/audit_vmem_pairing.py), a standard-library,
read-only comparison of specs, actions, retrieval, reconstruction, and saved PNG
hashes when available. It reports pre-eviction divergence separately from
expected post-eviction changes. Recorded PNG-byte differences are not a pixel
distance or a neural quality metric. It supplements, not replaces, inventory
validation. It never loads recovery checkpoints or rewrites old run metadata.
